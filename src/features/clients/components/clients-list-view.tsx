  ...
  const filteredWork = useMemo(() => {
    ...
    return allWorkAccounts.filter(
      (wa) =>
        ...
        wa.industry.toLowerCase().includes(q)
    );
  }, [search]);
        ...
        <TabsContent value="personal">
          <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    {[t("col.client"), t("col.email"), t("col.phone"), t("col.workAccounts"), t("col.joinedDate")].map((h) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <a
                            ...
                            className="text-slate-500 hover:text-[#28B8B1] transition-colors text-sm"
                          >
                            {client.email}
                          </a>
                        </td>
            ...
            <div className="border-t border-slate-50 px-5 py-2.5 text-xs text-slate-400">
              {filteredPersonal.length} {t("ofTotal", { total: mockClients.length })}
            </div>
          </div>
        </TabsContent>
          ...
          <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    {[t("col.company"), t("col.owner"), t("col.industry"), t("col.status"), t("col.createdDate")].map((h) => (
                      <th key={h} className="px-5 py-3.5 text-start text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">
                        {h}
                      </th>
                    ))}
                  ...
                  {filteredWork.length === 0 ? (
                    ...
                    filteredWork.map((wa, i) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400 font-mono">{wa.id}</p>
                            </div>
                          </div>
                        </td>

                        {/* Owner — Quick Link back to personal detail */}
                        <td className="px-5 py-3.5">
                          <button
                            onClick={() => navigateToClient(wa.ownerId)}
                        ...
                        {/* Status */}
                        <td className="px-5 py-3.5">
                          <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${waCls[wa.status]}`}>
            ...
            {/* Footer */}
            <div className="border-t border-slate-50 px-5 py-2.5 text-xs text-slate-400">
              {filteredWork.length} {t("ofTotal", { total: allWorkAccounts.length })}
            </div>
          </div>
        </TabsContent>
      </Tabs>
